...
TEST_CASE("OpenNonUniform", "[bspline]")
{
  ...
  SECTION("fit(...) - dense")
  {
    ...
    {
      REQUIRE_THAT(control_points.at(i), WithinRel(c_data.at(i)));
    }
    for (size_t i{0}; i < x_values.size(); i++)
    {
      REQUIRE_THAT(bspline.evaluate(x_values.at(i)), WithinRel(y_values.at(i)));
  ...
  SECTION("fit(...) - sparse")
  {
    ...
    std::uniform_real_distribution unif{
        big_knots.at(degree), big_knots.at(big_knots.size() - 1 - degree)
    };
    ...
    {
      REQUIRE_THAT(control_points.at(i), WithinRel(big_ctrl_pts.at(i), 1e-12));
    }
